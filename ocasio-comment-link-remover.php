<?php
/**
 * Plugin Name: Ocasio Comment Link Remover
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-comment-link-remover/
 * Description: Strips author website links and URLs from WordPress comments to eliminate backlink spam.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ocasio-comment-link-remover
 */
...
define('OCASIO_CLR_VERSION', '1.0.0');
define('OCASIO_CLR_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_CLR_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_clr_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_clr_activate() {
	if (get_option('ocasio_clr_enabled') === false) {
		$legacy = get_option('ko_clr_enabled', 1);
		update_option('ocasio_clr_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_clr_activate');

/**
 * 2. Core Logic: Remove Website URL Field & Backlink Spam
 */
function ocasio_clr_is_active() {
	$enabled = get_option('ocasio_clr_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_clr_enabled', 1);
	}
	return (int) $enabled === 1;
}

function ocasio_clr_remove_url_field($fields) {
	if (!ocasio_clr_is_active()) {
		return $fields;
	}
	...
	// Clean cookie consent label if present
...
add_filter('comment_form_default_fields', 'ocasio_clr_remove_url_field');

/**
 * Remove link wrapper around comment author names
 */
function ocasio_clr_disable_comment_author_links($author_link, $author) {
	if (ocasio_clr_is_active()) {
		return esc_html($author);
	}
	return $author_link;
}
add_filter('get_comment_author_link', 'ocasio_clr_disable_comment_author_links', 10, 2);

/**
 * 3. Settings Registration
 */
function ocasio_clr_register_settings() {
	register_setting('ocasio_clr_settings_group', 'ocasio_clr_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_clr_register_settings');

/**
 * 4. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_clr_register_menu() {
...
add_action('admin_menu', 'ocasio_clr_register_menu');

/**
 * 5. Action Links on Plugins Screen
 */
function ocasio_clr_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-plugins-main')) . '">' . esc_html__('Dashboard', 'ocasio-comment-link-remover') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_clr_action_links');

/**
 * 6. Conditional Admin Asset Enqueuing
 */
function ocasio_clr_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-clr-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_CLR_VERSION);
		wp_enqueue_script('ocasio-clr-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_CLR_VERSION, true);
		wp_localize_script('ocasio-clr-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-clr-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_clr_admin_assets');
